  ...
  upsert: (state, action) => {
    ...
    if (createdBy) {
      ...
      if (newCreatedBy.size === 0) {
        console.warning('Orphan detected:', branch);
      }
    }
...
Goblin.registerQuest(goblinName, 'wait-collect', function*(quest, next) {
  const cond = quest.goblin.getX('isCollecting');
  if (cond) {
    cond.wait(({collecting}) => {
      if (collecting) {
...
Goblin.registerQuest(goblinName, 'collect', function*(quest, root) {
  const state = quest.goblin.getState();
  ...
  if (!cond) {
    quest.goblin.setX(
      'isCollecting',
      quest.goblin.locks.createCondVariable({collecting: true})
    );
  }

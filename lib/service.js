...
class GarbageCollector {
  ...
  updateOwnership(
        ...
        if (parent) {
          ...
          if (branch === parent) {
            ...
            if (!state.has(`_subscriptions.feed-${feed}.branches.${parent}`)) {
              const error = `Missing parent ${parent} for branch ${branch} in the feed ${feed}`;
    ...
    if (skip > 0 && skip === parents.length * feeds.length) {
      ...
      return {state: immState, collected: true};
    }
...
const logicHandlers = {
  ...
  'upsert': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    newState.get(`${action.meta.branch}.public`, {}).forEach((v, k) => {
      const old = immState.get([action.meta.branch, 'public', k], null);
    ...
    for (const feed in list) {
      for (const parent of parents) {
        ...
        if (
          ...
          const error = `Attach impossible because the parent ${parent} is missing for the branch ${branch} in the feed ${feed}`;
          // throw new Error(error);
          console.warn(error);
        }

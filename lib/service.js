...
const notifyChanges = function () {
  for (const notify of Object.values(subscriptions)) {
    setImmediate(notify);
  }
};

let changeFeeds = new Goblin.Shredder({});
let goblins = new Goblin.Shredder({});
...
const logicHandlers = {
  ...
  'upsert': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    if (Object.keys(data).length !== 0 || !goblins.has(branch)) {
      goblins = goblins.set(branch, data);
    }

    if (!goblins.get(`${branch}.id`)) {
      goblins = goblins.set(`${branch}.id`, branch);
    }
  ...
  'remove-batch': (state, action) => {
    ...
    goblins = goblins.deleteAll(branches);
    return state;
  },
  'subscribe': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    action.get('branches').forEach((branch) => {
      ...
      state = update.state;
      if (!update.collected && !goblins.has(branch)) {
        goblins = goblins.set(branch, {id: branch});
      }
    });
...
const getChanges = (sub) =>
  new Goblin.Shredder().withMutations((changes) => {
    sub
      .get('branches', {})
      .forEach((_, key) => changes.set(key, goblins.get(key)));
  });
...
const handleChanges = (quest, feed) => {
  ...
  if (changeFeeds.get(feed, {}).equals(goblins)) {
    return;
  }

  const changes = getChanges(sub);
...
Goblin.registerQuest(goblinName, 'attach-to-parents', function (
  ...
  _check(quest, $msg);
  const goblinsSnapshot = goblins;
  const origState = quest.goblin.getState();
  let isAttached = false;
  if (goblinsSnapshot.has(branch)) {
    if (!Array.isArray(feeds)) {
      feeds = [feeds];
    }
  ...
  if (!isAttached && goblins.has(branch) && !gc.inFeeds(newState, branch)) {
...
Goblin.registerQuest(goblinName, 'get', function (quest, path, view, $msg) {
  ...
  if (!view) {
    return goblins.get(path, null);
  } else {
    const state = goblins.get(path, null);
    if (!state) {
      return null;
    }
...
Goblin.registerQuest(goblinName, 'query', function (
  ...
  let branches = goblins;
  if (feed) {
    const sub = getSubscription(state, feed);
    if (sub) {
      branches = getChanges(sub);
    }
  }
...
Goblin.registerQuest(goblinName, 'subscribe', function (
  ...
  //TODO: use array collector
  const hc = _.throttle(handleChanges, 100);
  subscriptions[feed] = () => hc(quest, feed);
...
const getMetrics = function (goblin) {
  ...
  metrics['entries.total'] = goblins.size;
  return metrics;
};

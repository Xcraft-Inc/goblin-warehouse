...
class GarbageCollector {
  ...
  inFeeds(state, branch) {
    return state.get(`_subscriptions`).some(s => s.hasIn(['branches', branch]));
  }
...
Goblin.registerQuest(goblinName, 'graph', function(quest, output) {
  ...
  state.get('_subscriptions').forEach((_, sub) => {
    ...
    state.get(`_subscriptions.${sub}.branches`).forEach((ownership, branch) => {
      ...
      const namespace = branch.split('@')[0];
      let bgcolor = '#eeeeee';
      ...
      const label =
        `"${branch}-${index}" [label=` +
        `<<table bgcolor="${bgcolor}" color="${bgcolor}" border="5" style="rounded" cellpadding="0" cellborder="0">` +
        `<tr><td colspan="2">${namespace}</td></tr>` +
        `<tr><td width="18" align="right" valign="top"><font point-size="6" color="#888888">id:</font></td>` +
        `<td width="100" align="left" valign="top"><font point-size="6" color="#888888">${branchLabel.join(
          "<br align='left'/>"
        )}<br align='left'/></font></td></tr>` +
        `<tr><td align="right" valign="top"><font point-size="6" color="#888888">meta:</font></td>` +
        `<td width="100" align="left" valign="top"><font point-size="6" color="#888888">state=${
          state.has(branch)
            ? 'true'
            : `</font><font color="#b01717">false</font><font point-size="6" color="#888888">`
        }, gen=${state.get(`_generations.${branch}`)}</font></td></tr>` +
        `</table>> fontname=Helvetica shape=plaintext]`;
      ...
      const entry = (left, right) =>
        `"${left}-${index}" -> "${right}-${index}" [color="#888888", arrowsize=.6]`;

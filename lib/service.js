...
const {regex, ArrayCollector} = require('xcraft-core-utils');
...
const logicHandlers = {
  '_init': (state, action) =>
    state.set('_feedsCollector', action.get('feedsCollector')),
...
const getChanges = (state, sub) =>
  new Goblin.Shredder().withMutations((changes) => {
    for (const branch of sub.get('branches').keys()) {
      ...
      if (hasView && branchState) {
        const view = sub.get(['views', branch]);
        changes.set(branch, getViewState(view, branchState));
      } else {
        changes.set(branch, branchState);
      }
    }
  });
...
const computePatch = (feed, state) => {
  ...
  return {state: newState, generation, _xcraftPatch: patch};
};

const handleFeedChanges = async (state, feed, evt) => {
  const sub = state.get(['_subscriptions', feed]);
  const changes = getChanges(state, sub);
  if (!changeFeeds.get(feed, {}).equals(changes)) {
    changeFeeds = changeFeeds.set(feed, changes);
    evt(`<${feed}>.changed`, computePatch(feed, changes.state));
  }
};

const handleChanges = async (quest, feed = null, resp = null) => {
  const state = quest.goblin.getState();

  const evt = resp
    ? (topic, ...args) => resp.events.send(`${goblinName}.${topic}`, ...args)
    : (...args) => quest.evt(...args);

  const feeds = feed ? [feed] : state.get('_patchFeeds').keys();

  for (const feed of feeds) {
    await handleFeedChanges(state, feed, evt);
  }
};
...
Goblin.registerQuest(goblinName, '_init', function (quest) {
  ...
  const evt = _.throttle(() => resp.events.send('<warehouse-changes>'), 100);

  quest.sub.local(
    '*::<warehouse-changes>',
    async () => await handleChanges(quest, null, resp)
  );
  quest.goblin.store.subscribe(evt);
  ...
  const onCollect = function* (collected, resp, next) {
    ...
    /* Generate subset of branches by feed */
    for (const branch in collected) {
      let feeds = gc.extractPatchFeeds(state, branch);
      feeds = feeds.concat(
        collected[branch] /* delete case */
          .filter((item) => item.feed && !feeds.includes(item.feed))
          .map((item) => item.feed)
      );
      ...
      for (const feed of feeds) {
        ...
        /* TODO: take the last entry (in the array) compatible with this feed */
        changes[feed][branch] = collected[branch];
      }
    }
  ...
  const feedsCollector = new ArrayCollector(resp, 100, onCollect);
  quest.do({feedsCollector});
});
...
Goblin.registerQuest(goblinName, 'syncChanges', async (quest, feed) => {
  ...
  await handleChanges(quest, feed);
});

...
const logicHandlers = {
  ...
  'unsubscribe': (state, action) => {
    ...
    state.get(`_subscriptions.feed-${feed}.branches`).forEach((_, branch) => {
      gc.unsubscribeBranch(state, branch, `feed-${feed}`);
    });
...
// TODO: add feed pre-selection support for optimize
...
/* This quest is used by killFeed, do not use quest.do here! */
...
/* This quest is used by killFeed, do not use quest.do here! */
function feedSubscriptionDel(quest, feed, branch, parents, $msg) {
  _check(quest, $msg);
  if (!parents) {
    parents = [branch];
  }
...
Goblin.registerQuest(goblinName, 'feed-subscription-del', feedSubscriptionDel);
...
/* Keep it atomic and sync */
Goblin.registerQuest(goblinName, 'killFeed', function (quest, feed, $msg) {
  _check(quest, $msg);
  feedSubscriptionDel(quest, feed, feed, feed, $msg);
  unsubscribe(quest, feed, $msg);
});

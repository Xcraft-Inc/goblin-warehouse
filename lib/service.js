...
const logicHandlers = {
  ...
  'feed.add': (state, action) => {
    return state.set (
      `_subscriptions.feed-${action.get ('feed')}.branches.${action.get ('branch')}`,
      true
    );
  },
  ...
  'feed.del-in-batch': (state, action) => {
    let branches = state.get (
      `_subscriptions.feed-${action.get ('feed')}.branches`
    );
...
const computePatch = state => {
  let delta;
  if (previousStates) {
    delta = diff (previousStates, state);
  } else {
    delta = diff (fromJS ({}), state);
  }
  previousStates = state;
  delta._xcraftPatch = true;
  return delta;
};

const handleChanges = _.throttle ((quest, feed) => {
  ...
  const delta = computePatch (changes.state);
  quest.evt (`${feed}.changed`, delta);
...
Goblin.registerQuest (goblinName, 'upsert', quest => {
  // Don't change this quest, or fix the goblin-core upsert short-circuit.
  quest.do ();
});
...
Goblin.registerQuest (goblinName, 'resend', (quest, feed) => {
  ...
  quest.evt (`${feed}.changed`, changes.state);
});
...
Goblin.registerQuest (goblinName, 'feed.del', (quest, feed, branch) => {
  const sub = getSubscription (quest.goblin.getState (), feed);
  if (!sub.get ('branches').has (branch)) {
    return false;
  }

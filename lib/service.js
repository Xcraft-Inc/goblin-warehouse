...
const computeBranchPatch = (feed, branch, state) => {
  ...
  const newState = patch
    ...
    : diff(fromJS({}), state); // FIXME: do not use diff() here
...
const handleFeedChanges = async (state, feed, branches, evt) => {
  ...
  if (!patch) {
    ...
    const {state} = getChanges(state, sub);
    evt(`<${feed}>.changed`, {state, generation, _xcraftPatch: false});
    return;
  }
...
Goblin.registerQuest(goblinName, '_init', function (quest) {
  ...
  const feedsAggregator = new MapAggregator(
    quest.newResponse(),
    100,
    onCollect
  );

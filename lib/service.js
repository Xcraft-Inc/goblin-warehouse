...
const logicHandlers = {
  ...
  'upsert': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    const aggregatorPut = () =>
      feeds.forEach(
        (feed) =>
          gc.inPatchFeed(state, feed, branch) &&
...
Goblin.registerQuest(goblinName, '_init', function (quest) {
  ...
  const onCollect = async (collected, resp) => {
    console.log(JSON.stringify(collected, ' ', 2));
  };
  const feedsAggregator = new MapAggregator(resp, 100, onCollect);

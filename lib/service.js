...
const logicHandlers = {
  ...
  'upsert': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    const hasDispatched = action.get('_goblinHasDispatched');
    ...
    if (!newState.get(['_generations', branch]) && generation) {
      newState = newState.set(
        ['_generations', branch, 'generation'],
        generation
      );
    }
    if (hasDispatched) {
      newState = newState.set(
        ['_generations', branch, 'hasDispatched'],
        hasDispatched
      );
    }
  ...
  'attach-to-parents': (state, action) => {
    ...
    try {
      ...
      if (generation) {
        state = state.set(['_generations', branch, 'generation'], generation);
      }
    }
  ...
  'graft': Goblin.Shredder.mutableReducer((state, action) => {
    ...
    const reverse = action.get('reverse') || false;
    const feedsAggregator = state.get('_feedsAggregator');

    const topdown = (state, branch) => {
      ...
      const branchState = state
        ...
        .set('children', {});
      let parents = branchState.get('parents');
      state = state.mergeDeep(
        `_subscriptions.${toFeed}.branches.${branch}`,
        branchState
      );
      feedsAggregator.put([toFeed, branch], true);
      for (const [parent] of parents) {
        if (parent !== branch) {
          state = topdown(state, parent);
        }
      }
      /* Fix children for the parents of branch */
      parents = state.get(
        ['_subscriptions', fromFeed, 'branches', branch, 'parents'] //
      );
      for (const [parent] of parents) {
        state = state.merge(
          ['_subscriptions', toFeed, 'branches', parent, 'children'],
          {[branch]: true}
        );
        feedsAggregator.put([toFeed, parent], true);
      }
      return state;
    };

    const bottomup = (state, branch) => {
      if (!state.has(['_subscriptions', fromFeed, 'branches', branch])) {
        return state;
      }
      const branchState = state
        .get(['_subscriptions', fromFeed, 'branches', branch])
        .set('parents', {});
      const children = branchState.get('children');
      state = state.mergeDeep(
        `_subscriptions.${toFeed}.branches.${branch}`,
        branchState
      );
      ...
      for (const [child] of children) {
        if (child !== branch) {
          state = bottomup(state, child);
        }
      }
      for (const [child] of children) {
        state = state.merge(
          ['_subscriptions', toFeed, 'branches', child, 'parents'],
          {[branch]: true}
        );
        feedsAggregator.put([toFeed, child], true);
      }
      return state;
    };

    return reverse ? bottomup(state, branch) : topdown(state, branch);
  }),
  'acknowledge': (state, action) => {
    const branch = action.get('branch');
    const generation = action.get('generation');
    if (generation === state.get(['_generations', branch, 'generation'])) {
    ...
    switch (type) {
      ...
      case 'delete':
        if (cnt === 1) {
          ...
          const line = s.state.getIn(['_lines', lineId]);
          if (line.size === 0) {
            s.state = s.state.deleteIn(['_lines', lineId]);
          }
          break;
        }
        --cnt;
...
const getChanges = (state, sub) =>
  new Goblin.Shredder().withMutations((changes) => {
    for (const branch of sub.get('branches').keys()) {
      if (
        branch.indexOf('@') === -1 ||
        state.get(['_generations', branch, 'hasDispatched'])
      ) {
...
const computeBranchPatch = (feed, branch, state) => {
  ...
  const newState = patch
    ? diff(previousBranchStates[feed][branch], state)
    : diff(null, state);
...
///   and/or
/// type: 'missionOrderTicket' OR ['actor1', 'actor2']
///
...
function unsubscribe(quest, feed, $msg) {
  ...
  if (!sub) {
    quest.log.warn(
      `Unavailable feed '${feed}'; maybe nothing has been attached to it`
    );
  }
...
Goblin.registerQuest(goblinName, 'status', function (quest) {
  ...
  state.get(`_generations`).forEach(({generation, hasDispatched}, branch) => {
    quest.log.info(`├─${branch}`);
    quest.log.info(`│ │─ gen: ${generation}`);
    quest.log.info(`│ └─ dis: ${hasDispatched}`);
  });
});

Goblin.registerQuest(goblinName, 'graph', async function (quest, output) {

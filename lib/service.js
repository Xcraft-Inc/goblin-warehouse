...
const {isImmutable, isShredder} = require('xcraft-core-shredder');
...
const trigHandleChanges = async (quest, feed) => {
  ...
  const branches = Array.from(sub.get('branches').keySeq());
  await store.syncChanges({feed, branches});
};
...
Goblin.registerQuest(goblinName, 'upsert', function (
  ...
  const state = quest.goblin.getState();
  const upsert = state.get(`${branch}._upsert`);
  ...
  const id = newState.get(`${branch}.id`);

  if (isImmutable(data) || isShredder(data)) {
    data = data.set('id', id);
  } else {
    data.id = id;
  }

  quest.defer(async () => quest.cmd('warehouse-store.upsert', {branch, data}));
});
...
Goblin.registerQuest(
  ...
  async (
    ...
    if (feed) {
      ...
      const sub = getSubscription(state, feed);
      if (sub) {
        branches = Array.from(sub.get('branches').keySeq());
      }
    }

    const store = quest.getAPI('warehouse-store');
    return await store.query({feed, branches, ids, type, filter, view});
  }
);
...
Goblin.registerQuest(goblinName, 'resend', async (quest, feed, $msg) => {
  ...
  const store = quest.getAPI('warehouse-store');
  const branches = Array.from(sub.get('branches').keySeq());
  await store.resend({feed, branches});
});
...
function unsubscribe(quest, feed, $msg) {
  ...
  quest.go('warehouse-store.clearPreviousState', {feed});
...
Goblin.registerQuest(goblinName, 'subscribe', function (
  ...
  quest.do();
  const hc = _.throttle(trigHandleChanges, 100);
...
Goblin.registerQuest(goblinName, 'sync-changes', async (quest, feed) => {
  await trigHandleChanges(quest, feed);
});

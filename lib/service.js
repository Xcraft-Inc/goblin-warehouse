...
const branchesGen = new Map();
const states = new Map();
let changeFeeds = new Goblin.Shredder({});

const incBranchGen = (branch) => {
  if (branchesGen.has(branch)) {
    ++branchesGen[branch];
  } else {
    branchesGen[branch] = 0;
  }
};
...
const logicHandlers = {
  ...
  'upsert': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    const hasStatesBranch = states.has(branch);
    ...
    if (Object.keys(data).length !== 0 || !hasStatesBranch) {
      states.set(branch, Goblin.Shredder.toImmutable(data));
      //newState = newState.set(branch, data);
    }

    if (!states.has(branch)) {
      states.set(branch, fromJS({id: branch}));
    } else if (!states.get(branch).get('id')) {
      states.set(branch, states.get(branch).set('id', branch));
    }
  ...
  'acknowledge': (state, action) => {
    ...
    if (generation === state.get(`_generations.${branch}`)) {
      branchesGen.delete(branch);
      states.delete(branch);
      return state.del(`_generations.${branch}`);
    }
    return state;
  },
  'remove-batch': (state, action) => {
    ...
    branches.forEach((branch) => states.delete(branch));
    return state;
  },
  'subscribe': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    action.get('branches').forEach((branch) => {
      ...
      state = update.state;
      if (!update.collected && !states.has(branch)) {
        states.set(branch, fromJS({id: branch}));
      }
    });
...
const getChanges = (state, sub) => {
  ...
  sub
    .get('branches', {})
    .forEach((_, key) => changes.set(key, states.get(key)));
  return changes;
};
...
const computePatch = (feed, state, subscriptions) => {
  let newState;
  const patch = !!previousStates[feed];

  if (patch) {
    const previousSubs = previousStates[feed].subscriptions;

    /* List of identical branches */
    const identical = {};
    previousStates[feed].branchesGen.forEach((gen, branch) => {
      if (!previousSubs.has(branch)) {
        return;
      }
      const _gen = branchesGen[branch];
      if (gen === _gen) {
        identical[branch] = true;
      }
    });

    let _previousState = fromJS({});
    for (const branch of previousStates[feed].state.keys()) {
      if (!identical[branch]) {
        _previousState = _previousState.set(
          branch,
          previousStates[feed].state.get(branch)
        );
      }
    }
    //const _previousState = previousStates[feed].state.filter(
    //  (_, branch) => !identical[branch]
    //);
    let _newState = fromJS({});
    for (const branch of state.keys()) {
      if (!identical[branch]) {
        _newState = _newState.set(branch, state.get(branch));
      }
    }
    //const _newState = state.filter((_, branch) => !identical[branch]);

    newState = diff(_previousState, _newState);
    // newState = diff(previousStates[feed].state, state)
  } else {
    newState = Object.fromEntries(state.entries());
  }

  const generation = getNextGeneration(feed);
  previousStates[feed] = {
    state,
    branchesGen: new Map(branchesGen),
    generation,
    subscriptions,
  };
...
const handleChanges = (quest, feed) => {
  ...
  const diff = computePatch(feed, changes, sub.get('branches').state);
  if (!diff.size) {
    return;
  }
...
Goblin.registerQuest(goblinName, 'attach-to-parents', function (
  ...
  if (!isAttached && states.has(branch) && !gc.inFeeds(newState, branch)) {
...
Goblin.registerQuest(goblinName, 'has', function (quest, path, $msg) {
  ...
  return !path ? states.has(branch) : states.get(branch).has(path);
});
...
Goblin.registerQuest(goblinName, 'get', function (quest, path, view, $msg) {
  ...
  if (!view) {
    return !path ? states.get(branch) : states.get(branch).get(path, null);
  } else {
    const state = !path
      ? states.get(branch)
      : states.get(branch).get(path, null);
    if (!state) {
      return null;
    }
...
Goblin.registerQuest(goblinName, 'resend', function (quest, feed, $msg) {
  ...
  const changes = getChanges(state, sub);
  const generation = getNextGeneration(feed);
  const subscriptions = sub.get('branches').state;
  previousStates[feed] = {
    state: changes,
    branchesGen: new Map(branchesGen),
    generation,
  ...
  quest.evt(
    `<${feed}>.changed`,
    fromJS({
      state: Object.fromEntries(changes.entries()),
      generation,
      _xcraftPatch: false,
    })
  );
...
Goblin.registerQuest(goblinName, 'checkOrphan', function (quest) {
  ...
  subscriptions.forEach((_, sub) => {
    ...
    branches.forEach((ownership, branch) => {
      ...
      parents
        .map((_, parent) => parent)
        .filter((parent) => parent !== 'new' && !states.has(parent))
        .forEach((parent) => {
...
function buildFullLabel(state, branch, ownOwner, index) {
  ...
  const hasState = () => (states.has(branch) ? 'true' : toRed('false'));
  const getGen = () =>
...
const getMetrics = function (goblin) {
  ...
  metrics['entries.total'] = states.size;
  return metrics;
};

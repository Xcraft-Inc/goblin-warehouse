...
const collect = (state, feed, branch, list) => {
  if (!list[feed]) {
    list[feed] = {};
  }
  ...
  const isOwn = state
    .get(`_subscriptions`)
    .some((_, feed) => state.has(`_subscriptions.${feed}.branches.${branch}`));
  if (!isOwn) {
...
const unsubscribeBranch = (state, branch, feed) => {
  let list;

  if (!feed) {
    list = extractFeeds(state, branch);
  } else {
    list = {
      [feed]: {
        [branch]: true,
  ...
  while (loop) {
    ...
    for (const feed in list) {
      for (const branch in list[feed]) {
        state = collect(state, feed, branch, list);
      }
...
const extractFeeds = (state, branch) => {
  const list = {};
...
const logicHandlers = {
  ...
  'delete-branch': (state, action) => {
    const branch = action.get('branch');
    return unsubscribeBranch(state, branch);
  },
  ...
  'detach-from-parents': (state, action) => {
    const branch = action.get('branch');
    let parents = action.get('parents');
    ...
    const list = extractFeeds(state, branch);
    for (const feed in list) {
      ...
      if (
        ...
        state = unsubscribeBranch(state, branch, feed);
      }
    }
...
Goblin.registerQuest(goblinName, 'feed-subscription-del', function*(
  ...
  yield quest.me.detachFromParents({branch, parents});
  return true;
});

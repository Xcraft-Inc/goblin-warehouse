...
const computePatch = (feed, state, subscriptions) => {
  ...
  if (patch) {
    ...
    const _previousState = fromJS({}).withMutations((_previousState) => {
      for (const branch of previousStates[feed].state.keys()) {
        if (!identical[branch]) {
          _previousState.set(branch, previousStates[feed].state.get(branch));
        }
      }
    });

    const _newState = fromJS({}).withMutations((_newState) => {
      for (const branch of state.keys()) {
        if (!identical[branch]) {
          _newState.set(branch, state.get(branch));
        }
      }
    });

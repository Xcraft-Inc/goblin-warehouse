...
const logicHandlers = {
  upsert: (state, action) => {
    ...
    newState.getIn ([action.meta.branch, 'public'], fromJS ({})).forEach ((v,k) => {
      const old = state.getIn ([action.meta.branch, 'public', k], null);
      publicChangeFeeds = setPropertyChanged ([action.meta.branch, k], hasChanged (old, v));
      return true;
    });
    return newState;
  },
  subscribe: (state, action) => {
    return state.setIn (['_subscriptions', action.meta.feed], fromJS ({
  ...
  registerUnsubscribe: (state, action) => {
    return state.updateIn (['_subscriptions', action.meta.feed], fromJS ({

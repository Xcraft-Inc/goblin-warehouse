...
Goblin.registerQuest(goblinName, 'graph', function*(
  ...
  if (layout) {
    if (!['circo', 'dot', 'fdp', 'neato', 'osage', 'twopi'].includes(layout)) {
  ...
  state.get('_subscriptions').forEach((_, sub) => {
    ...
    state.get(`_subscriptions.${sub}.branches`).forEach((ownership, branch) => {
      ...
      const label = buildLabel(ownOwner);
  ...
  if (svg) {
    yield viz.save(
      path.join(output, `warehouse.${timestamp}.svg`),
      {
        totalMemory: parseInt(memory) || 64e6,
      },
      next
    );
  }

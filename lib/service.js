  ...
  'unsubscribe': Goblin.Shredder.mutableReducer((state, action) => {
    ...
    branches.forEach((_, branch) => {
      if (state.has(`_subscriptions.feed-${feed}.branches.${branch}`)) {
        state = gc.unsubscribeBranch(state, branch, `feed-${feed}`);
      }
    });
...
const handleChanges = (quest, feed) => {
  ...
  const branches = sub.get('branches', {});

  /* Filter the warehouse state with only the branches of our feed */
  const changes = new Goblin.Shredder().withMutations((changes) => {
    branches.forEach((_, key) => changes.set(key, state.get(key)));
  });
...
Goblin.registerQuest(goblinName, 'unsubscribe', function (quest, feed, $msg) {
  _check(quest, $msg);
  const state = quest.goblin.getState();
  const sub = getSubscription(state, feed);
  if (sub) {

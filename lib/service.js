...
const logicHandlers = {
  '_init': (state) => state.del('_deleted'),
  ...
  'upsertClear': (state, action) =>
    state.del(`${action.get('branch')}._upsert`),
  'upsert': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    if (Object.keys(data).length !== 0 || !immState.has(branch)) {
      ...
      if (branch !== 'warehouse-store') {
        newState = newState.set(`${branch}._upsert`, {generation});
      }
    }
      ...
      for (const feed in list) {
        for (const parent of parents) {
          ...
          state = state
            ...
            .set(['_store', 'feeds', feed, branch], null);
        ...
        if (view && state.has(['_subscriptions', feed, 'branches', branch])) {
          state = state
            .set(['_subscriptions', feed, 'views', branch], view)
            .set(['_store', 'feeds', feed, branch], view);
        }
      }
    } finally {
      if (generation) {
        state = state
          .set(['_generations', branch], generation)
          .set(['_store', 'gens', branch], generation);
      }
    }
  ...
  'acknowledge': (state, action) => {
    ...
    if (generation === state.get(['_generations', branch])) {
      return state
        .del(branch)
        .del(['_generations', branch])
        .set(['_deleted', branch], generation);
    }
    return state;
  },
  ...
  'unsubscribe': Goblin.Shredder.mutableReducer((state, action) => {
    ...
    return state.del(['_subscriptions', feed]).del(['_store', 'feeds', feed]);
  }),
...
let prevSubs = new Goblin.Shredder({});
let prevPatches = new Goblin.Shredder({});
let prevGens = new Goblin.Shredder({});

const subChanges = watt(function* (quest, resp, next) {
  const state = quest.goblin.getState();
  const subs = state.get('_store.feeds');
  const patches = state.get('_patchFeeds');
  const gens = state.get('_store.gens');
  const dels = state.get('_deleted');
  let changes;
  let send = false;

  if (!subs.equals(prevSubs)) {
    if (!changes) {
      changes = new Goblin.Shredder();
    }
    changes = changes.set(`subs`, subs);
    prevSubs = subs;
    send = true;
  }

  if (!patches.equals(prevPatches)) {
    if (!changes) {
      changes = new Goblin.Shredder();
    }
    changes = changes.set(`patches`, patches);
    prevPatches = patches;
    send = true;
  }

  if (!gens.equals(prevGens)) {
    if (!changes) {
      changes = new Goblin.Shredder();
    }
    changes = changes.set(`gens`, gens);
    prevGens = gens;
    send = true;
  }
  ...
  quest.do();
  yield resp.command.send('warehouse-store.syncSubs', {changes}, next);
});
...
Goblin.registerQuest(goblinName, '_init', function (quest) {
  ...
  const sc = _.throttle(subChanges, 50);

  quest.sub.local(
    '*::<warehouse-sub-changes>',
    async () => await sc(quest, resp)
  );
  quest.goblin.store.subscribe(() =>
    resp.events.send('<warehouse-sub-changes>')
  );
});
...
Goblin.registerQuest(goblinName, 'upsert', function (
  ...
  quest.do();
  const state = quest.goblin.getState();
  const toUpsert = state.get(`${branch}._upsert`);
  quest.dispatch('upsertClear', {branch});
  return toUpsert;
});
...
Goblin.registerQuest(goblinName, 'get', async (quest, path, view, $msg) => {
  _check(quest, $msg);
  // HACK: the get must be sent directly to the store
  const store = quest.getAPI('warehouse-store');
  return await store.get({path, view});
});
...
Goblin.registerQuest(
  ...
  async (
    ...
    _check(quest, $msg);
    // HACK: the query must be sent directly to the store
    const store = quest.getAPI('warehouse-store');
    return await store.query({feed, ids, type, filter, view});
  }
);
...
Goblin.registerQuest(goblinName, 'resend', async (quest, feed, $msg) => {
  _check(quest, $msg);
  const store = quest.getAPI('warehouse-store');
  await store.resend({feed});
});

function unsubscribe(quest, feed, $msg) {
  _check(quest, $msg);
  quest.dispatch('unsubscribe', {feed});
  quest.evt('<feed-subscriptions-changed>');
}

...
const xLog = require('xcraft-core-log')('warehouse');
...
class GarbageCollector {
  ...
  _collect(state, feed, branch, list, autoRelease = false) {
    ...
    if (state.has(['_patchFeeds', feed])) {
      ...
      feedsAggregator.put([feed, branch], false); /* delete */
    }
    ...
    if (!isOwn) {
      const generation = state.get(['_generations', branch, 'generation']);
      if (!generation) {
        xLog.warn(
  ...
  inPatchFeed(state, feed, branch) {
    return state.has(['_patchFeeds', feed]) && this.inFeed(state, feed, branch);
  }
  ...
  /**
   * Extract the list of feeds with patches when the branch exists.
   *
   * @param {*} state - Warehouse state
   * @param {*} branch - Branch to search in the feeds
   * @returns {Array} list of feeds
   */
  extractPatchFeeds(state, branch) {
    return state
      .get('_patchFeeds')
      .filter((_, feed) =>
        state.has(['_subscriptions', feed, 'branches', branch])

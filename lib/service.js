...
const handleChanges = (quest, feedName) => {
  ...
  quest.evt (`${feedName}.changed`, payload);
...
goblin.registerQuest ('save', function* (quest) {
  yield quest.saveState ();
});

goblin.registerQuest ('load', function* (quest) {
...
goblin.registerQuest ('resend', (quest, msg) => {
  const state = goblin.getState ();
  const feedName = msg.get ('feedName');
  const sub = getSubscription (state, feedName);
  if (!sub) {
    return;
  }

...
const updateNestedSubscriptions = (state, feed, branch, checkList) => {
  state
    .keySeq()
    .filter(
      id =>
        state.has(`${id}.createdBy`) &&
        state.get(`${id}.createdBy`).includes(branch)
    )
    .forEach(id => {
      /* keep somewhere items where at least one other subscription exists
       * for this feed. Maybe at least one owner will stay and in this case
       * the subscription stays too.
       */
      if (
        state
          .get(`${id}.createdBy`)
          .some(
            nid =>
              nid !== branch &&
              state.get(`_subscriptions.${feed}.branches.${nid}`, false)
          )
      ) {
        checkList.add(id);
      }

      state = state.set(`_subscriptions.${feed}.branches.${id}`, false);
      state = updateNestedSubscriptions(state, feed, id, checkList);
    });
  return state;
};

const updateSubscriptions = (state, {owners, newOwners, branch}) => {
  /* 1. Loop on all feeds (_subscriptions) */
  const feeds = state.get('_subscriptions').keySeq();
  for (const feed of feeds) {
    const checkList = new Set();
    let tagged = false;
    /* 2. Loop for all removed createdBy */
    owners
      /* The old id must have a subscription for the current feed */
      .filter(oid => state.get(`_subscriptions.${feed}.branches.${oid}`, false))
      ...
      .forEach(() => {
        /* 3. Tag for removing the branch (id) for each removed createdBy */
        state = state.set(`_subscriptions.${feed}.branches.${branch}`, false);
        tagged = true;
      });

    if (tagged) {
      state = updateNestedSubscriptions(state, feed, branch, checkList);
    }

    /* Restore subscriptions when at least one owner persists. */
    for (const id of checkList) {
      if (
        state
          .get(`${id}.createdBy`)
          .some(owner =>
            state.get(`_subscriptions.${feed}.branches.${owner}`, false)
          )
      ) {
        state = state.set(`_subscriptions.${feed}.branches.${id}`, true);
      }
    }
  }
...
const logicHandlers = {
  ...
  upsert: (state, action) => {
    ...
    if (isCreating === true) {
      newCreatedBy = newCreatedBy.push('', 'new');
    } else if (isCreating === false) {
      newCreatedBy = newCreatedBy.unpush('', 'new');
    }
    if (createdBy) {
    ...
    if (TTL) {
      ...
      newCreatedBy = newCreatedBy.push('', `TTL-${getTimestampForTTL(TTL)}`);
    }
  ...
  'remove-created-by': (state, action) => {
    ...
    owners
      .filter(owner => newCreatedBy.includes(owner))
      .forEach(owner => {
        newCreatedBy = newCreatedBy.unpush('', owner);
      });
  ...
  'update-subscriptions': (state, action) => {
    const branch = action.get('branch');
    const owners = action.get('owners');
    const newOwners = action.get('newOwners');

    return updateSubscriptions(state, {owners, newOwners, branch});
  },
  'remove-batch': (state, action) => {
    ...
    toClean.forEach(id => {
      ...
      garbaged = updateSubscriptions(garbaged, {
        owners: oldCreatedBy,
        newOwners: newCreatedBy,
        branch: id,
      });
    });
    return garbaged;
...
const handleChanges = (quest, feed) => {
  ...
  const changes = state
    .filter((_, key) => sub.has(`branches.${key}`))
    .maps(v =>
      isProduction /* Clean createdBy in development and remove the key in production */
        ? v.delete('createdBy')
        : v.set(
            'createdBy',
            v.get('createdBy').map(v => v.replace(/^TTL-.*/, 'TTL'))
          )
    );
...
Goblin.registerQuest(goblinName, 'collect', function collect(
  ...
  if (hashCode === quest.goblin.getX('hashCode')) {
    return;
  }
  ...
  const currentTimestamp = getTimestampForTTL(0);
  let collectable = branches
    .where(
    ...
    .where(col =>
      col.owners.every(owner => {
        //check special's cases:
        if (owner === 'none') {
          return true;
        }
        // The branch is creating...
        if (owner === 'new') {
          return false;
        }

        if (owner.startsWith('TTL-')) {
          const TTL = parseFloat(owner.replace('TTL-', ''));
          return currentTimestamp > TTL;
        }

        //default case
        return !state.has(owner);
      })
    )
    .union(
      branches
  ...
  if (collectable.length === 0) {
    ...
    quest.goblin.setX('hashCode', quest.goblin.getState().hashCode());
    quest.log.info('collect [done]');
    return;
  }

  collectable = _.uniqBy(collectable, 'id');
  const _branches = collectable.map(col => col.id);
   ...
   */
  quest.dispatch('remove-batch', {branches: _branches});

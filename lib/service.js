...
const logicHandlers = {
  ...
  subscribe: (state, action) => {
    return state.set (['_subscriptions', action.get ('feed')], {
      branches: action.get ('branches'),
      unsubscribe: null,
    });
  },
  'feed.add': (state, action) => {
    const sub = getSubscription (state, action.get ('feed'));
    const newBranches = sub.get ('branches').push (action.get ('branch'));
    const newSub = sub.set ('branches', newBranches);
    return state.set (['_subscriptions', action.get ('feed')], newSub);
  },
...
Goblin.registerQuest (goblinName, 'feed.add', quest => {
  quest.do ();
});

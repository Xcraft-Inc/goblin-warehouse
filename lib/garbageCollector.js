...
class GarbageCollector {
  ...
  _collect(state, feed, branch, list, autoRelease = false) {
    ...
    state = state
      ...
      .del(`_store.feeds.${feed}.${branch}`);
    delete list[feed][branch];

    /* Remove empty feed */
    if (state.get(`_subscriptions.${feed}.branches`).size === 0) {
      state = state.del(`_subscriptions.${feed}`).del(`_store.feeds.${feed}`);
    }
      ...
      for (const parent of parents) {
        ...
        if (parent) {
          ...
          if (branch === parent) {
            ...
            state = state
              .set(`_subscriptions.${feed}.branches.${parent}`, _ownership)
              .set(
                `_store.feeds.${feed}.${branch}`,
                state.get(`_subscriptions.${feed}.views.${branch}`) || null
              );
          }
        ...
        state = state
          .set(`_subscriptions.${feed}.branches.${branch}`, ownership)
          .set(
            `_store.feeds.${feed}.${branch}`,
            state.get(`_subscriptions.${feed}.views.${branch}`) || null
          );

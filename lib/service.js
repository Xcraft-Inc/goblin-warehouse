...
const computePatch = (feed, state, branchesGen) => {
  ...
  if (patch) {
    ...
    const identical = previousStates[feed].branchesGen.filter((gen, branch) => {
      if (!previousStates[feed].state.has(branch)) {
        return false;
      }
    ...
    const _previousState = previousStates[feed].state.filter(
      (_, branch) => !identical.has(branch)
    );
    const _newState = state.filter((_, branch) => !identical.has(branch));

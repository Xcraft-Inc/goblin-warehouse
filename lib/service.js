...
Goblin.registerQuest(goblinName, 'graph', function(quest, output) {
  ...
  state.get('_subscriptions').forEach((_, sub) => {
    ...
    state.get(`_subscriptions.${sub}.branches`).forEach((ownership, branch) => {
      ...
      const label = `"${branch}-${index}" [label=<${
        branch.split('@')[0]
      }<br/><font point-size='6' color='#999999'><br align='left'/>${branchLabel.join(
        "<br align='left'/>"
      )}<br align='left'/></font>> shape=Mrecord fontname=Helvetica]`;
      ...
      Object.keys(ownership.children)
        .filter(
          child =>
            !graph.statements.includes(
              `"${child}-${index}" -> "${branch}-${index}" [color="#999999"]`
            )
        )
        .forEach(child =>
      ...
      Object.keys(ownership.parents)
        .filter(
          parent =>
            !graph.statements.includes(
              `"${branch}-${index}" -> "${parent}-${index}" [color="#999999"]`
            )
        )
        .forEach(parent =>
          graph.statements.push(
  ...
  viz.save(path.join(output, `warehouse.${Date.now()}.svg`));
  fs.writeFileSync(path.join(output, `warehouse.${Date.now()}.dot`), viz.dot());
});

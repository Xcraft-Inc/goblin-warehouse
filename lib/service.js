...
class GarbageCollector {
  ...
  _collect(state, feed, branch, list) {
    ...
    if (!isOwn) {
      ...
      const generation = state.get(`_generation.${branch}`);
      if (generation) {
        state = state.del(`_generation.${branch}`);
...
const logicHandlers = {
  ...
  upsert: Goblin.Shredder.mutableReducer((state, action) => {
    ...
    if (!newState.get(`_generation.${branch}`) && generation) {
      newState = newState.set(`_generation.${branch}`, generation);
    }
...
Goblin.registerQuest(goblinName, 'del-creator', function*(quest, branch, $msg) {
  _check(quest, $msg);
  quest.do();
  yield quest.me.detachFromParents({branch, parents: ['new']});
});

      ...
      for (const parent of parents) {
        ...
        if (parent) {
          ...
          if (branch === parent) {
            ...
            /* Ensure that this parent exists at least in one feed */
            if (!this.inFeeds(state, parent)) {
              ++skip;
              continue;
            }
...
Goblin.registerQuest(goblinName, 'graph', function(quest, output) {
  ...
  state.get('_subscriptions').forEach((_, sub) => {
    ...
    state.get(`_subscriptions.${sub}.branches`).forEach((ownership, branch) => {
      ...
      const toRed = value =>
        `<font point-size="6" color="#b01717">${value}</font>`;
      ...
      let label =
        `"${branch}-${index}" [label=<` +
        `<font point-size="6" color="#888888">` +
        ...
        `  </tr>` +
        `  <tr>` +
        `    <td width="18" align="right" valign="top"><b>&nbsp;</b>id:</td>` +

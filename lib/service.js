  ...
  'remove-batch': (state, action) => {
    ...
    toClean.forEach(id => {
      ...
      const newCreatedBy = createdBy.filter(id => {
        ...
        // (x) 1. Récupérer la liste des createdBy supprimés (branches.indexOf(id) !== -1)
        oldCreatedBy.push(id);
        return false;
      });
      garbaged = garbaged.set(`${id}.createdBy`, newCreatedBy.toArray());
      // (x) 2. parcourir tous les feeds (_subscriptions)
      const feeds = garbaged.get('_subscriptions').keySeq();
      for (const feed of feeds) {
        // 3. Pour chaque oldCreatedBy
        oldCreatedBy.forEach(oid => {
          if (garbaged.has(`_subscriptions.${feed}.branches.${oid}`)) {
            // 4. Supprimer id pour chaque createdBy supprimé
...
Goblin.registerQuest(
  ...
  (quest, feed, branches, $msg) => {
    throw new Error('Dangerous call, subscriptions leaks possible');
    _check(quest, $msg);
    quest.do();
  }
);

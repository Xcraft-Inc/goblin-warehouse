...
const logicHandlers = {
  ...
  'upsert': Goblin.Shredder.mutableReducer((state, action, immState) => {
    ...
    if (update.collected) {
      update.state = update.state.set(
        `_branchesGen.${branch}`,
        update.state.get(`_branchesGen.${branch}`, 0) + 1
      );
      return update.state;
    }
    ...
    newState = newState.set(
      `_branchesGen.${branch}`,
      newState.get(`_branchesGen.${branch}`, 0) + 1
    );
    return newState;
  }),
...
const computePatch = (feed, state, branchesGen) => {
  let newState;
  const patch = !!previousStates[feed];

  if (patch) {
    const previousStateKeys = Object.fromEntries(
      Array.from(previousStates[feed].state.keys()).map((v) => [v, true])
    );

    /* List of identical branches */
    const identical = {};
    previousStates[feed].branchesGen.forEach((gen, branch) => {
      if (!previousStateKeys[branch]) {
        return;
      }
      const _gen = branchesGen.get(branch);
      if (gen === _gen) {
  ...
  previousStates[feed] = {state, branchesGen, generation};
...
const handleChanges = (quest, feed) => {
  ...
  const branchesGen = state.get('_branchesGen');

  // notify global state on bus
  quest.evt(
    `<${feed}>.changed`,
    computePatch(feed, changes.state, branchesGen.state)
  );
};
...
Goblin.registerQuest(goblinName, 'resend', function (quest, feed, $msg) {
  ...
  const changes = getChanges(state, sub);
  const branchesGen = state.get('_branchesGen').state;
  const generation = getNextGeneration(feed);
  previousStates[feed] = {state: changes.state, branchesGen, generation};
